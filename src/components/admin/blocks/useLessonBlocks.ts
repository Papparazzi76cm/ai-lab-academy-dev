...
export function useLessonBlocks(lessonId: string) {
  ...
  const pendingSaveBlocksRef = useRef<{
  ...
  // Core save function with concurrency requestId protection
  const saveBlocksToSupabase = useCallback(
    async (
      blocksToSave: LessonBlockItem[],
      targetLessonId: string,
      requestId: number,
    ): Promise<boolean> => {
      if (!targetLessonId) return false;

      // Validate all blocks (content and settings). Do NOT filter out invalid blocks.
      let invalidCount = 0;
      for (const b of blocksToSave) {
        const check = validateBlock(b.type, b.content_json, b.settings_json);
        if (!check.valid) {
          invalidCount++;
        }
      }
      ...
      if (invalidCount > 0) {
        if (
          ...
          targetLessonId === currentLessonIdRef.current &&
          isMountedRef.current
        ) {
          setSaveStatus("error");
          toast.error("Cambios sin guardar: corrige los bloques con errores");
        }
        return false;
      }
      ...
      try {
        ...
        // Concurrency check: ONLY modify saveStatus if this is still the active request for active lesson
        if (
          requestId === currentRequestIdRef.current &&
          targetLessonId === currentLessonIdRef.current &&
          isMountedRef.current
  ...
  useEffect(() => {
    if (currentLessonIdRef.current !== lessonId) {
      const oldLessonId = currentLessonIdRef.current;
      currentRequestIdRef.current += 1;

      // Flush any pending save for old lesson without polluting new lesson's state
      if (pendingSaveBlocksRef.current && pendingSaveTimeoutRef.current) {
        clearTimeout(pendingSaveTimeoutRef.current);
        const { blocks: oldBlocks, requestId: oldReqId } = pendingSaveBlocksRef.current;
        pendingSaveBlocksRef.current = null;
        void saveBlocksToSupabase(oldBlocks, oldLessonId, oldReqId);
      }
  ...
  }, [lessonId, saveBlocksToSupabase]);

  // Sync query data to local state WITHOUT overwriting active local edits
  useEffect(() => {
    if (fetchedBlocks) {
  ...
  const flushPendingSave = useCallback(async (): Promise<boolean> => {
    ...
    if (pendingSaveBlocksRef.current) {
      const {
        ...
        requestId,
      } = pendingSaveBlocksRef.current;
      pendingSaveBlocksRef.current = null;
      return await saveBlocksToSupabase(blocksToSave, targetLessonId, requestId);
    }

    const hasInvalidBlock = blocks.some(
      (b) => !validateBlock(b.type, b.content_json, b.settings_json).valid,
    );
    if (hasInvalidBlock) {
      setSaveStatus("error");
      toast.error("Cambios sin guardar: corrige los bloques con errores");
  ...
  }, [blocks, saveBlocksToSupabase, saveStatus]);
  ...
  const scheduleAutosave = useCallback(
    (newBlocks: LessonBlockItem[]) => {
      ...
      pendingSaveBlocksRef.current = { blocks: newBlocks, lessonId, requestId: reqId };
      ...
      pendingSaveTimeoutRef.current = setTimeout(() => {
        if (pendingSaveBlocksRef.current) {
          ...
          pendingSaveBlocksRef.current = null;
          void saveBlocksToSupabase(blocksToSave, targetLessonId, requestId);
        }
      }, 1000);
    },
    [lessonId, saveBlocksToSupabase],
    ...
    (
      ...
      setBlocks((prev) => {
        const next = prev.map((b) => {
          ...
          const check = validateBlock(b.type, updatedContent, updatedSettings);
  ...
  const reorderBlocks = useCallback(
    async (reorderedIds: string[]) => {
      ...
      try {
        ...
        if (error) throw error;
        if (reqId === currentRequestIdRef.current && isMountedRef.current) {
          setSaveStatus("saved");
          setTimeout(() => {
            if (reqId === currentRequestIdRef.current && isMountedRef.current) {
              setSaveStatus("idle");
